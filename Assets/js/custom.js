...
/*
 * the dynamicField object to house our calls.
 */
var dynamicField = {
    ...
    add_flexible_layout : function( $tr ){
        // vars
        var $new_tr = $tr.clone( false );

        console.log($tr, $new_tr, $new_tr.find('.dynamicField-fc-meta input'));

        // remove sub fields
        //$new_tr.find('.field_option_flexiblecontent').not('[data-key="acfcloneindex"]').remove();
        $new_tr.find('.another-field').not('[data-key="acfcloneindex"]').remove();


        // reset layout meta values
        $new_tr.find('.dynamicField-fc-meta input').val('');
    ...
    wipe_layout : function( $tr ){
        ...
        // vars
        var old_key = $tr.attr('ç'),
            new_key = this.get_uniqid();
    ...
    duplicate_flexible_layout : function( $tr ){
    ...
    delete_flexible_layout : function( $tr ){

        // validate
        if( $tr.siblings('tr[class="field_option_flexiblecontent"]').length == 0 ) {
...
$( document ).ready(function() {
    ...
    /*
     * the select dropdown that chooses
     * which fieldtype you wish to use
     */
    ...
    // remove field when click btn delete
    ...
    /*
     * when wishing to add a flexible content layout box on
     */
    $(document).on('click', '[data-name="dynamicField-fc-add"]', function( e ){

        e.preventDefault();

        dynamicField.add_flexible_layout( $(this).closest('.field_option') );

    });

    /*
     * when wishing to delete a flexible content layout box on
     */
    $(document).on('click', '[data-name="dynamicField-fc-delete"]', function( e ){

        e.preventDefault();

        dynamicField.delete_flexible_layout( $(this).closest('.field_option') );

    });

    /*
     * when wishing to duplicate a flexible content layout box on
     */
    $(document).on('click', '[data-name="dynamicField-fc-duplicate"]', function( e ){

        e.preventDefault();

        dynamicField.duplicate_flexible_layout( $(this).closest('.field_option') );
