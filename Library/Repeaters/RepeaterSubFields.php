...
class RepeaterSubFields implements RepeaterSubFieldsContract {
...
//        }
            ...
            foreach($row as $subKey => $column)
            {
                ...
                if($getFieldTranslationData == true) {
                    ...
                    if(isset($column->field_translation->unique_id)) {
                        ...
                        $field = $this->fieldByUniqueIdTranslationUniqueId($column->unique_id,
    ...
    public function collectFieldsInRows($rows)
    {
        ...
        foreach ($rows as $row)
        {
            $newRows[] = FieldTypeSorter::handle($row, $this->getFieldTranslationData, $this->options);
        }
    ...
    public function sortThroughChildrenFields($fieldCollection)
    {
        ...
        $subFields = FieldTypeSorter::handle($subFields, $this->getFieldTranslationData, $this->options);
        return $subFields;
    }
    ...
    public function getRepeaterChildren($field)
    {
        ...
        if($this->getFieldTranslationData === true){
            $fieldCollection = Field::where('parent', '=', $field['unique_id'])
                ->orderBy('order')
                ->with([
                    'fieldTranslation' => function ($query) use ($options) { // repeaterFieldTranslation
        ...
        if($this->getFieldTranslationData === false){
            $fieldCollection = Field::where('parent', '=', $field['unique_id'])->orderBy('order')->get();
        }
